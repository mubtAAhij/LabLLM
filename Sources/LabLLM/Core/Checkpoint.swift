...
enum CheckpointError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .directoryCreationFailed(let n): return String(format: String(localized: "checkpoint.error.directory-creation-failed", defaultValue: "Couldn't create a folder for checkpoint '%@'.", bundle: .main, comment: "Error when creating checkpoint directory"), n)
        case .weightsWriteFailed(let r): return String(format: String(localized: "checkpoint.error.weights-write-failed", defaultValue: "Couldn't write model weights: %@", bundle: .main, comment: "Error when writing checkpoint model weights"), r)
        case .weightsReadFailed(let r): return String(format: String(localized: "checkpoint.error.weights-read-failed", defaultValue: "Couldn't read model weights: %@", bundle: .main, comment: "Error when reading checkpoint model weights"), r)
        case .metaMissing: return String(localized: "checkpoint.error.metadata-missing", defaultValue: "This checkpoint is missing its metadata file.", bundle: .main, comment: "Error when checkpoint metadata file is missing")
        case .shapeMismatch(let r): return String(format: String(localized: "checkpoint.error.shape-mismatch", defaultValue: "Checkpoint doesn't match the current model shape: %@", bundle: .main, comment: "Error when checkpoint shape does not match current model"), r)
        }
    }
}
...
enum Checkpoint {
    struct Meta: Codable {
        ...
        var valLoss: Float
        var createdAt: Date
        var method: String = String(localized: "checkpoint.meta.method.pretraining", defaultValue: "Pretraining", bundle: .main, comment: "Default training method name")     // "Pretraining", "SFT (LoRA)", "SFT (full)", "DPO"
    ...
    static func save(model: GPT, meta: Meta, name: String, hardware: HardwareInfo? = nil,
        ...
        } catch { throw CheckpointError.weightsWriteFailed(String(format: String(localized: "checkpoint.error.metadata-write-failed", defaultValue: "metadata: %@", bundle: .main, comment: "Metadata write error detail prefix"), error.localizedDescription)) }
    ...
    static func loadOptimizerSnapshot(from dir: URL, step: Int) throws -> TrainingOptimizerSnapshot? {
        ...
        do {
            ...
            throw CheckpointError.weightsReadFailed(String(format: String(localized: "checkpoint.error.optimizer-read-failed", defaultValue: "optimizer: %@", bundle: .main, comment: "Optimizer read error detail prefix"), error.localizedDescription))
        }
    }
    ...
    static func saveQuantized(from dir: URL, bits: Int, groupSize: Int = 64, hardware: HardwareInfo) throws
        ...
        var qMeta = meta
        qMeta.method = meta.method + String(format: String(localized: "checkpoint.summary.quantized-bits", defaultValue: " · quantized %d-bit", bundle: .main, comment: "Checkpoint summary quantization bits"), bits)
