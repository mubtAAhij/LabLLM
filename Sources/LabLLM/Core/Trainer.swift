...
final class Trainer: ObservableObject {
    ...
    @Published var statusMessage: String = String(
        localized: "trainer.state.idle",
        defaultValue: "Idle",
        comment: "Trainer state label when no run is active"
    )
    ...
    func pause() { pauseRequested = true; publish { self.isPaused = true; self.statusMessage = String(
        localized: "trainer.state.paused",
        defaultValue: "Paused",
        comment: "Trainer state label when run is paused"
    ) } }
    func resume() { pauseRequested = false; publish { self.isPaused = false; self.statusMessage = String(
        localized: "trainer.state.training",
        defaultValue: "Training",
        comment: "Trainer state label when run is training"
    ) } }
    ...
    func start(gptConfig: GPTConfig, trainConfig: TrainConfig, tokenizer: Tokenizer, corpus: String,
        ...
        beginSession(mode: .pretrain, maxSteps: trainConfig.maxSteps, statusMessage: String(
            localized: "trainer.status.preparing",
            defaultValue: "Preparing…",
            comment: "Status text while trainer prepares run resources"
    ...
    private func run(_ gptConfig: GPTConfig, _ tc: TrainConfig, _ tokenizer: Tokenizer, _ corpus: String,
        ...
        if let resumeFrom, let meta = try? Checkpoint.loadMeta(from: resumeFrom),
           let loaded = try? Checkpoint.loadModel(from: resumeFrom, meta: meta) {
            ...
            if restoredOptimizerSnapshot != nil, restoredTrainRNGState != nil {
                ...
                publish { self.statusMessage = String(format: String(
                    localized: "trainer.status.resumed-from-checkpoint",
                    defaultValue: "Resumed from %@",
                    comment: "Status text after resuming from a checkpoint file"
                ), "\(resumeFrom.lastPathComponent)") }
            } else {
                publish { self.statusMessage = String(
                    localized: "trainer.status.legacy-checkpoint-no-optimizer-rng",
                    defaultValue: "Loaded legacy checkpoint weights; optimizer/RNG state unavailable",
                    comment: "Status message indicating legacy checkpoint without optimizer or RNG state"
                ) }
            }
        } else {
        ...
        publish { self.hasModel = true; self.statusMessage = String(
            localized: "trainer.mode.training",
            defaultValue: "Training",
            comment: "Mode label for generic training run"
        ) }
        ...
        for s in startStep ... tc.maxSteps {
            ...
            if s % tc.sampleEvery == 0 { emitLiveSample(model: model, tokenizer: tokenizer, step: s, method: String(
                localized: "trainer.mode.pretraining",
                defaultValue: "Pretraining",
                comment: "Mode label for pretraining run"
            )) }
        ...
        publish {
            ...
            self.runMethod = String(
                localized: "trainer.progress.pretraining",
                defaultValue: "Pretraining",
                comment: "Progress title for pretraining run"
            )
            self.statusMessage = self.stopRequested ? String(format: String(
                localized: "trainer.status.stopped-at-step-progress-saved",
                defaultValue: "Stopped at step %d — progress saved",
                comment: "Completion message when training stops and saves progress"
            ), self.step)
                : String(
                    localized: "trainer.status.training-done",
                    defaultValue: "Training done",
                    comment: "Completion message when training finishes"
                ) + (final != nil ? " " + String(
                    localized: "trainer.status.checkpoint-saved-suffix",
                    defaultValue: "— checkpoint saved",
                    comment: "Suffix appended to completion message when checkpoint is saved"
                ) : "")
        }
    }
    ...
    func startSFT(gptConfig: GPTConfig, trainConfig tc: TrainConfig, tokenizer: Tokenizer,
        ...
        guard !isTraining else { return }
        beginSession(mode: .sft, maxSteps: tc.maxSteps, statusMessage: String(
            localized: "trainer.status.preparing-finetuning",
            defaultValue: "Preparing fine-tuning…",
            comment: "Status text while preparing fine-tuning resources"
        ), datasetName: datasetName)
        queue.async {
    ...
    private func runSFT(_ gptConfig: GPTConfig, _ tc: TrainConfig, _ tokenizer: Tokenizer,
        ...
        if let resumeFrom, let meta = try? Checkpoint.loadMeta(from: resumeFrom),
           let loaded = try? Checkpoint.loadModel(from: resumeFrom, meta: meta) {
            ...
            if restoredOptimizerSnapshot != nil, restoredTrainRNGState != nil {
                ...
                publish { self.statusMessage = String(
                    localized: "trainer.status.legacy-checkpoint-no-optimizer-rng-finetuning",
                    defaultValue: "Loaded legacy checkpoint weights; optimizer/RNG state unavailable",
                    comment: "Fine-tuning status when legacy checkpoint lacks optimizer or RNG state"
                ) }
            }
        ...
        self.model = model; self.tokenizer = tokenizer
        publish { self.hasModel = true; self.statusMessage = model.hasLoRA ? String(
            localized: "trainer.mode.finetuning-lora",
            defaultValue: "Fine-tuning (LoRA)",
            comment: "Mode label for LoRA fine-tuning"
        ) : String(
            localized: "trainer.mode.finetuning-full",
            defaultValue: "Fine-tuning (full)",
            comment: "Mode label for full fine-tuning"
        ) }
        let startTime = Date(); var lastReport = Date()
        ...
        for s in startStep ... maxSteps {
            ...
            do {
                ...
                let result = try withError { () throws -> ([MLXArray], ModuleParameters) in
                    ...
                    guard !values.isEmpty else {
                        throw TrainingFailure(message: String(
                            localized: "trainer.error.no-sft-loss",
                            defaultValue: "MLX returned no SFT loss. Try disabling LoRA for this run or reducing batch/context size.",
                            comment: "Error message when MLX returns no supervised fine-tuning loss"
                        ))
                    }
                ...
                publish {
                    self.errorMessage = String(format: String(
                        localized: "trainer.error.finetuning-stopped",
                        defaultValue: "Fine-tuning stopped: %@",
                        comment: "Error message when fine-tuning stops with underlying error"
                    ), "\(error.localizedDescription)")
                    self.statusMessage = String(
                        localized: "trainer.error.finetuning-needs-attention",
                        defaultValue: "Fine-tuning needs attention",
                        comment: "Headline for fine-tuning attention-required alert"
                    )
                }
                break
            }
            ...
            if s % sampleEvery == 0 { emitLiveSample(model: model, tokenizer: tokenizer, step: s, method: String(
                localized: "trainer.progress.finetuning",
                defaultValue: "Fine-tuning",
                comment: "Progress title for fine-tuning run"
            )) }
        ...
        publish {
            self.isTraining = false
            self.runCompleted = !self.stopRequested
            self.runMethod = model.hasLoRA ? String(
                localized: "trainer.mode.sft-lora",
                defaultValue: "SFT (LoRA)",
                comment: "Mode label for supervised fine-tuning with LoRA"
            ) : String(
                localized: "trainer.mode.sft-full",
                defaultValue: "SFT (full)",
                comment: "Mode label for full supervised fine-tuning"
            )
            self.statusMessage = self.stopRequested ? String(format: String(
                localized: "trainer.status.stopped-at-step-progress-saved-sft",
                defaultValue: "Stopped at step %d — progress saved",
                comment: "SFT completion message when stopped and progress is saved"
            ), self.step)
                : String(
                    localized: "trainer.status.finetuning-done",
                    defaultValue: "Fine-tuning done",
                    comment: "Completion message when fine-tuning finishes"
                ) + (final != nil ? " " + String(
                    localized: "trainer.status.checkpoint-saved-suffix-finetuning",
                    defaultValue: "— checkpoint saved",
                    comment: "Suffix indicating checkpoint was saved after fine-tuning"
                ) : "")
        }
    }
    ...
    func startDPO(trainConfig tc: TrainConfig, examples: [PreferenceExample], hardware: HardwareInfo) {
        guard !isTraining, let policyBase = model, let tokenizer = tokenizer else {
            publish { self.errorMessage = String(
                localized: "trainer.error.dpo-requires-finetuned-model",
                defaultValue: "DPO needs a fine-tuned model in memory first — run SFT, then DPO.",
                comment: "Guidance error when DPO starts without a fine-tuned model loaded"
            ) }
            return
        }
        beginSession(mode: .dpo, maxSteps: tc.maxSteps, statusMessage: String(
            localized: "trainer.status.preparing-dpo",
            defaultValue: "Preparing DPO…",
            comment: "Status text while preparing DPO run"
        ))
        queue.async {
    ...
    private func runDPO(_ tc: TrainConfig, _ examples: [PreferenceExample], _ policy: GPT,
        ...
        publish { self.statusMessage = String(
            localized: "trainer.progress.dpo-training",
            defaultValue: "DPO training",
            comment: "Progress title for DPO training run"
        ) }
        let startTime = Date(); var lastReport = Date()
        ...
        publish {
            ...
            self.runCompleted = !self.stopRequested
            self.runMethod = String(
                localized: "trainer.mode.dpo",
                defaultValue: "DPO",
                comment: "Mode label for direct preference optimization run"
            )
            self.statusMessage = self.stopRequested ? String(format: String(
                localized: "trainer.status.stopped-at-step-progress-saved-dpo",
                defaultValue: "Stopped at step %d — progress saved",
                comment: "DPO completion message when stopped and progress is saved"
            ), self.step)
                : String(
                    localized: "trainer.status.dpo-done",
                    defaultValue: "DPO done",
                    comment: "Completion message when DPO run finishes"
                ) + (final != nil ? " " + String(
                    localized: "trainer.status.checkpoint-saved-suffix-dpo",
                    defaultValue: "— checkpoint saved",
                    comment: "Suffix indicating checkpoint was saved after DPO"
                ) : "")
        }
    }
    ...
    func restore(session: TrainingSession) {
        ...
        statusMessage = session.step > 0 ? String(format: String(
            localized: "trainer.status.restored-run-at-step",
            defaultValue: "Restored %@ run at step %d",
            comment: "Status text after restoring a training session and step"
        ), "\(session.method)", session.step) : String(
            localized: "trainer.state.idle-restored",
            defaultValue: "Idle",
            comment: "Trainer state label set to idle after restoration path"
        )
    }
    ...
    func clearSession(mode: RunMode) {
        ...
        statusMessage = String(
            localized: "trainer.state.idle-reset",
            defaultValue: "Idle",
            comment: "Trainer state label set to idle on reset path"
        )
    }
    ...
    func unloadModel() {
        ...
        publish {
            ...
            self.statusMessage = String(
                localized: "trainer.state.idle-default",
                defaultValue: "Idle",
                comment: "Default trainer state label when no run is active"
            )
        }
    }
    ...
    func loadForSampling(model: GPT, tokenizer: Tokenizer) {
        ...
        publish { self.hasModel = true; self.runIsLoRA = model.hasLoRA; self.statusMessage = String(
            localized: "trainer.status.loaded-checkpoint",
            defaultValue: "Loaded checkpoint",
            comment: "Status text when checkpoint loads successfully"
        ) }
    }
    ...
    private func saveCheckpoint(model: GPT, config: GPTConfig, tokenizer: Tokenizer, step: Int, loss: Float,
        ...
        do {
            ...
            publish { self.errorMessage = String(format: String(
                localized: "trainer.error.could-not-save-checkpoint",
                defaultValue: "Couldn't save checkpoint: %@",
                comment: "Error message when saving checkpoint fails"
            ), "\(error.localizedDescription)") }
            return nil
        }
    }
