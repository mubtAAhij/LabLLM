...
struct HFHubDataset: Identifiable, Decodable, Hashable, Sendable {
    ...
    var summary: String {
        ...
        return task?.replacingOccurrences(of: "_", with: " ").capitalized ?? String(localized: "hugging-face.dataset.public-dataset-label", defaultValue: "Public Hugging Face dataset", comment: "Source label for public Hugging Face dataset entries")
    }
    var license: String { String(localized: "hugging-face.dataset.see-card-action", defaultValue: "See dataset card", comment: "Action text to open dataset card page") }
}
...
final class HFHubBrowser: ObservableObject {
    ...
    var pinned: [HFHubDataset] {
        ...
        case .corpus:
            return [
                ...
                .init(id: "Salesforce/wikitext", title: String(localized: "hugging-face.dataset.wikitext-2-raw", defaultValue: "WikiText-2 Raw", comment: "Preset dataset display name"), downloadSize: "4.7 MB", tags: ["text-generation"]),
                .init(id: "wikimedia/wikipedia", title: String(localized: "hugging-face.dataset.simple-english-encyclopedia", defaultValue: "Simple English Encyclopedia", comment: "Preset dataset display name"), downloadSize: "~300 MB", tags: ["text-generation"]),
                .init(id: "wikimedia/wikipedia", title: String(localized: "hugging-face.dataset.goodwiki", defaultValue: "GoodWiki", comment: "Preset dataset display name"), downloadSize: String(localized: "hugging-face.dataset.goodwiki.size-varies", defaultValue: "Varies by snapshot", comment: "Dataset size note for snapshot-dependent corpus"), tags: ["text-generation"]),
                .init(id: "karpathy/tiny_shakespeare", title: "Tiny Shakespeare", downloadSize: "1.1 MB", tags: ["text-generation"]),
                .init(id: "ccdv/arxiv-summarization", title: String(localized: "hugging-face.dataset.arxiv-abstracts", defaultValue: "arXiv Abstracts", comment: "Preset dataset display name"), downloadSize: "~240 MB", tags: ["text-generation"]),
                .init(id: "DanFosing/public-domain-poetry", title: String(localized: "hugging-face.dataset.public-domain-poetry", defaultValue: "Public Domain Poetry", comment: "Preset dataset display name"), downloadSize: "94.2 MB", tags: ["text-generation"]),
            ]
        case .fineTune:
            return [
                .init(id: "HuggingFaceTB/everyday-conversations-llama3.1-2k", title: String(localized: "hugging-face.dataset.everyday-conversations-2k", defaultValue: "Everyday Conversations 2k", comment: "Preset fine-tuning dataset display name"), estimatedRows: 2_260, tags: ["conversation"]),
                .init(id: "HuggingFaceH4/no_robots", title: String(localized: "hugging-face.dataset.no-robots", defaultValue: "No Robots", comment: "Preset fine-tuning dataset display name"), estimatedRows: 10_000, tags: ["instruction"]),
                .init(id: "databricks/databricks-dolly-15k", title: String(localized: "hugging-face.dataset.dolly-15k", defaultValue: "Dolly 15k", comment: "Preset fine-tuning dataset display name"), estimatedRows: 15_011, tags: ["instruction"]),
                .init(id: "OpenAssistant/oasst1", title: String(localized: "hugging-face.dataset.open-assistant-oasst1", defaultValue: "Open Assistant OASST1", comment: "Preset fine-tuning dataset display name"), estimatedRows: 161_443, tags: ["conversation"]),
                .init(id: "HuggingFaceTB/smoltalk", title: String(localized: "hugging-face.dataset.smoltalk", defaultValue: "SmolTalk", comment: "Preset fine-tuning dataset display name"), estimatedRows: 1_000_000, tags: ["conversation"]),
                .init(id: "openai/gsm8k", title: String(localized: "hugging-face.dataset.gsm8k", defaultValue: "GSM8K", comment: "Preset fine-tuning dataset display name"), estimatedRows: 8_792, tags: ["instruction", "math"]),
                .init(id: "grammarly/coedit", title: String(localized: "hugging-face.dataset.coedit", defaultValue: "CoEdIT", comment: "Preset fine-tuning dataset display name"), estimatedRows: 82_000, tags: ["instruction"]),
                .init(id: "sahil2801/CodeAlpaca-20k", title: String(localized: "hugging-face.dataset.codealpaca-20k", defaultValue: "CodeAlpaca 20k", comment: "Preset fine-tuning dataset display name"), estimatedRows: 20_022, tags: ["instruction", "code"]),
                .init(id: "rajpurkar/squad", title: String(localized: "hugging-face.dataset.squad", defaultValue: "SQuAD", comment: "Preset fine-tuning dataset display name"), estimatedRows: 87_599, tags: ["instruction"]),
                .init(id: "HuggingFaceH4/ultrachat_200k", title: String(localized: "hugging-face.dataset.ultrachat-200k", defaultValue: "UltraChat 200k", comment: "Preset fine-tuning dataset display name"), estimatedRows: 207_865, tags: ["conversation"]),
            ]
        }
    }
    ...
    func select(_ dataset: HFHubDataset?) {
        ...
        Task {
            ...
            readme = (try? await Task.detached { try await HFHubClient.readme(repo: dataset.id) }.value) ?? String(localized: "hugging-face.dataset.no-readme", defaultValue: "No README was published for this dataset.", comment: "Fallback text when dataset README is unavailable")
        }
    }
...
enum HFHubClient {
    static func search(query: String, limit: Int, offset: Int) async throws -> [HFHubDataset] {
        ...
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else { throw ConversationImportError.network(String(localized: "hugging-face.search.failed", defaultValue: "Couldn't search Hugging Face datasets.", comment: "Error text when dataset search request fails")) }
    ...
    static func files(repo: String, path: String? = nil, recursive: Bool = true) async throws -> [HFHubFile] {
        ...
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else { throw ConversationImportError.network(String(format: String(localized: "huggingface-hub.error.read-files-for-repo", defaultValue: "Couldn't read the files for %@", comment: "Network error message when listing files for a Hugging Face repository fails"), repo)) }
    ...
    static func readme(repo: String) async throws -> String {
        guard let url = HFDownloader.url(repo: repo, filePath: "README.md") else { throw ConversationImportError.network(String(localized: "huggingface-hub.error.invalid-dataset-repository", defaultValue: "Invalid dataset repository.", comment: "Error message for invalid dataset repository")) }
        let (data, response) = try await URLSession.shared.data(from: url)
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode), let text = String(data: data, encoding: .utf8) else { throw ConversationImportError.network(String(localized: "huggingface-hub.error.load-dataset-readme", defaultValue: "Couldn't load the dataset README.", comment: "Error message when dataset README cannot be loaded")) }
        return text
    }
    ...
    private static func viewerRequest<T: Decodable>(path: String, query: [String: String], as type: T.Type) async throws -> T {
        ...
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
            throw ConversationImportError.network(String(localized: "huggingface-hub.error.dataset-viewer-read-failed", defaultValue: "Hugging Face Dataset Viewer couldn't read this dataset.", comment: "Error message when Hugging Face dataset viewer fails"))
        }
