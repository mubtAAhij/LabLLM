...
enum ConversationImportError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noValidRows: return String(
            localized: "conversation-import.error.no-valid-rows",
            defaultValue: "No rows in this file matched a recognized conversation format.",
            comment: "Error shown when no recognized conversation rows are found"
        )
        case .network(let r): return String(format: String(
            localized: "conversation-import.error.network",
            defaultValue: "Network error: %@",
            comment: "Network error with reason during conversation import"
        ), "\(r)")
        case .badStatus(let code): return String(format: String(
            localized: "conversation-import.error.bad-status",
            defaultValue: "Server returned status %d. Check the repo/file path.",
            comment: "HTTP status error when importing conversation data"
        ), code)
        }
    }
}
    ...
    static func parseIMessageDatabase(at url: URL) throws -> [[ChatMessage]] {
        ...
        guard sqlite3_open_v2(url.path, &database, SQLITE_OPEN_READONLY, nil) == SQLITE_OK, let database else {
            throw ConversationImportError.network(String(
                localized: "conversation-import.messages-db.error.open",
                defaultValue: "Couldn't open chat.db. In System Settings, grant LabLLM Full Disk Access, then choose ~/Library/Messages/chat.db.",
                comment: "Error when Messages chat database cannot be opened"
            ))
        }
        ...
        guard sqlite3_prepare_v2(database, query, -1, &statement, nil) == SQLITE_OK, let statement else {
            throw ConversationImportError.network(String(
                localized: "conversation-import.messages-db.error.read",
                defaultValue: "Couldn't read messages from this chat database.",
                comment: "Error when reading from selected chat database fails"
            ))
        }
...
enum HFDownloader {
    ...
    static func download(repo: String, filePath: String, progress: @escaping @Sendable (Int64, Int64?) -> Void = { _, _ in }) async throws -> String {
        guard let url = url(repo: repo, filePath: filePath) else {
            throw ConversationImportError.network(String(
                localized: "conversation-import.github.error.invalid-path",
                defaultValue: "Invalid repo or file path.",
                comment: "Error for invalid GitHub repo or file path"
            ))
        }
        ...
        guard let text = String(data: data, encoding: .utf8) else {
            throw ConversationImportError.network(String(
                localized: "conversation-import.github.error.invalid-utf8",
                defaultValue: "Downloaded file wasn't valid UTF-8 text (it may be a Parquet/Arrow file rather than raw JSONL — try a different file path).",
                comment: "Error when downloaded dataset is not valid UTF-8 text"
            ))
        }
        return text
    }
