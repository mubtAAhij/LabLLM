...
enum NavSection: String, CaseIterable, Identifiable {
    ...
    var group: String {
        switch self {
        case .welcome: return String(localized: "content.sidebar.group.home", defaultValue: "HOME", comment: "Sidebar section header for home-related pages")
        case .model, .dataset, .recipes, .fineTuneData: return String(localized: "content.sidebar.group.build", defaultValue: "BUILD", comment: "Sidebar section header for build-related pages")
        case .training, .sampling, .chat, .checkpoints: return String(localized: "content.sidebar.group.run", defaultValue: "RUN", comment: "Sidebar section header for run-related pages")
        case .xray, .embeddings, .estimator, .hardware: return String(localized: "content.sidebar.group.analyze", defaultValue: "ANALYZE", comment: "Sidebar section header for analysis-related pages")
        case .server, .roadmap, .settings: return String(localized: "content.sidebar.group.system", defaultValue: "SYSTEM", comment: "Sidebar section header for system pages")
        }
    }
...
struct ContentView: View {
    ...
    private var navigationGroups: [String] { [String(localized: "content.sidebar.group.home-filter", defaultValue: "HOME", comment: "Filter group label for home section"), String(localized: "content.sidebar.group.build-filter", defaultValue: "BUILD", comment: "Filter group label for build section"), String(localized: "content.sidebar.group.run-filter", defaultValue: "RUN", comment: "Filter group label for run section"), String(localized: "content.sidebar.group.analyze-filter", defaultValue: "ANALYZE", comment: "Filter group label for analyze section"), String(localized: "content.sidebar.group.system-filter", defaultValue: "SYSTEM", comment: "Filter group label for system section")] }
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .automatic) {
                Button { showTutorial() } label: { Image(systemName: "questionmark.circle") }
                    .help(String(localized: "content.actions.replay-tutorial", defaultValue: "Replay tutorial", comment: "Action title to replay onboarding tutorial"))
            }
        }
    ...
    private var modeBadge: some View {
        HStack(spacing: 6) {
            ...
            Text(prefs.mode.label + " " + String(localized: "content.status.mode-suffix", defaultValue: "mode", comment: "Suffix appended to mode name in status text")).font(.caption.weight(.medium))
        }
        ...
        if trainer.isTraining {
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    Text(String(format: String(localized: "content.training.status.progress-step", defaultValue: "%d%% · Step %d/%d", comment: "Training status showing progress percent and current step"), Int((trainer.progress * 100).rounded()), trainer.step, trainer.maxSteps)).font(.caption).monospacedDigit()
                    Text(String(format: String(localized: "content.training.status.loss-format", defaultValue: "loss %.3f", comment: "Training status format string for loss value"), trainer.trainLoss))
